...
def analyze_log():
    ...
    result = analyze_log_with_ai(combined_log, failure, defect_class, station, bu, keywords=keywords, exclude_id=exclude_id)
...
def history_query():
    ...
    records = query.order_by(
        case((DefectReport.root_cause.isnot(None), 0), else_=1),
        DefectReport.created_at.desc()
    ).limit(3).all()
