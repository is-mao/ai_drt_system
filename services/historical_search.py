...
def search_similar_failures(failure, station=None, bu=None, exclude_id=None, limit=5):
    ...
    try:
        ...
        if station:
            ...
            station_results = query.filter(DefectReport.station == station).order_by(
                DefectReport.record_time.desc()
            ).limit(limit).all()
        ...
        if len(results) < limit:
            ...
            more_results = query.filter(
                DefectReport.id.notin_(existing_ids) if existing_ids else True
            ).order_by(DefectReport.record_time.desc()).limit(remaining).all()
